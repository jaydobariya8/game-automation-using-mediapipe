    ...
    while True:
        ...
        if landmarks:
            if game_started:
                ...
                # Show clear instruction to the user
                cv2.putText(
                    img,
                    'Join both hands in front of you to start the game.',
                    (5, h - 10),
        ...
        cv2.imshow('Game Automation - Pose Control', img)
        k = cv2.waitKey(1) & 0xFF
        # Allow user to exit with Esc or q
        if k == 27 or k == 113:
            break
    cap.release()
